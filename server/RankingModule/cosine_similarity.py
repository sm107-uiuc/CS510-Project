...
class CosineSimilarity:
    def __init__(self, query_json, results_json) -> None:
        ...
        #print(self.query_vector, "data type: ", str(type(self.query_vector[0])))
        #print(self.results_matrix, "data type: ", str(type(self.results_matrix[0])))
    ...
    def calculate_cosine_similarity(self):
        ...
        a_norm = np.linalg.norm(self.results_matrix, axis=1)
        b_norm = np.linalg.norm(self.query_vector)
        out = (self.results_matrix @ self.query_vector) / (a_norm * b_norm)
        out  = [round(i,8) for i in out]
        #replace nan with 0
